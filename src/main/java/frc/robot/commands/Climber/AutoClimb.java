...
public class AutoClimb extends SequentialCommandGroup {
  ...
  public AutoClimb(Climber climber, Pivots pivots) {
    ...
    addCommands(
      ...
      new InstantCommand(()-> pivots.setPositionPivots(-41)).withTimeout(2),
      new ClimbDown(climber, 1000, 1000).withTimeout(2).andThen(new PivotRelative(pivots, 20).withTimeout(1)),
      new ClimbUp(climber, 40000, 40000).withTimeout(1).andThen(new PivotRelative(pivots, -10).withTimeout(1)),
      new WaitCommand(2),
      new PivotRelative(pivots, 90, true).withTimeout(2),
      new ClimbUp(climber).withTimeout(4),
      new PivotRelative(pivots, -20).withTimeout(1),
      new ClimbDown(climber, 190000, 190000).withTimeout(1),
      new PivotRelative(pivots, -100).withTimeout(2),
